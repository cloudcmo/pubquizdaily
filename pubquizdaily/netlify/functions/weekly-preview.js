...
async function buildWhenlyPromo(fridayISO) {
  ...
  const peek = peekWhenly(questions);
  try {
    const { text, model } = await generateWhenlyTeaser(questions);
    return { teaser: text, peek, note: `Whenly teaser generated by ${model}` };
  } catch (err) {
    console.error('Whenly AI teaser failed, using fallback:', err.message);
    return { teaser: fallbackWhenlyTeaser(questions), peek, note: `Whenly teaser FELL BACK to template: ${err.message}` };
  }
}
...
async function buildWhatWordPromo(fridayISO) {
  ...
  const peek = peekWhatWord(questions);
  try {
    const { text, model } = await generateWhatWordTeaser(questions);
    return { teaser: text, peek, note: `What Word teaser generated by ${model}` };
  } catch (err) {
    console.error('What Word AI teaser failed, using fallback:', err.message);
    return { teaser: fallbackWhatWordTeaser(questions), peek, note: `What Word teaser FELL BACK to template: ${err.message}` };
  }
}
...
async function buildGroupiePromo(fridayISO) {
  ...
  const peek = peekGroupieGrid(groups, fridayISO);
  try {
    const { text, model } = await generateGroupieTeaser(groups);
    return { teaser: text, peek, note: `Groupie teaser generated by ${model}` };
  } catch (err) {
    console.error('Groupie AI teaser failed, using fallback:', err.message);
    return { teaser: fallbackGroupieTeaser(groups), peek, note: `Groupie teaser FELL BACK to template: ${err.message}` };
  }
}
...
// Hexadec. Template teaser, but a REAL peek: the day's sixteen tiles, which
// are the whole puzzle and give nothing away, because the four words are
// hidden among them by construction. Costs one 12KB fetch of a public static
// asset and no AI call. Fails to the plain block if the site is unreachable,
// which it is until the game is deployed.
async function buildHexadecPromo(fridayISO) {
  const teaser = 'Sixteen lettered tiles hide four four-letter words that use every tile exactly. The trouble is that far more words can be made than can be used, so three good ones can leave you holding nonsense.';
  let peek = '';
  try {
    const res = await fetch(`${HEXADEC_URL}/days.js`, { signal: AbortSignal.timeout(8000) });
    if (res.ok) peek = peekHexadecTiles(await res.text(), fridayISO);
  } catch (err) {
    console.error('Hexadec peek failed, block still renders:', err.message);
  }
...
// ── Puzzle peeks ─────────────────────────────────────────────────────────────
// A fragment of the ACTUAL puzzle for the send day, drawn in email-safe table
// HTML. No images, so nothing is blocked or broken by a client that strips
// them, and no extra AI call, so a peek costs nothing per send. The data is
// already fetched to write each teaser: this just shows it instead of
// describing it. Every peek returns '' rather than throwing, and the block
// renders exactly as before when it does.

// Deterministic shuffle so a rebuild produces the same grid as the preview.
...
function peekLabel(text) {
  return `<div style="font-family:Arial,Helvetica,sans-serif;font-size:11px;font-weight:700;letter-spacing:0.07em;text-transform:uppercase;color:#8a857d;padding-bottom:8px;">${escapeHtml(text)}</div>`;
}

// Groupie: the sixteen words as a 4x4 grid, shuffled so the four hidden
// groups are never betrayed by their position. This is the whole game in one
// glance, and it is different every single week by construction.
function peekGroupieGrid(groups, seedISO) {
  try {
    const words = groups.flatMap(g => g.words);
    if (words.length !== 16) return '';
    const shuffled = seededShuffle(words, `groupie-${seedISO}`);

    const cell = w => `<td width="25%" align="center" bgcolor="#f2eee7" style="background-color:#f2eee7;border-radius:7px;padding:10px 2px;font-family:Arial,Helvetica,sans-serif;font-size:11px;font-weight:700;letter-spacing:0.02em;text-transform:uppercase;color:#3a3a3a;word-break:break-word;">${escapeHtml(w)}</td>`;

    const rows = [0, 4, 8, 12]
      .map(i => `<tr>${shuffled.slice(i, i + 4).map(cell).join('')}</tr>`)
      .join('');

    return peekLabel("Today's grid") +
      `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate;border-spacing:4px;margin:0 0 16px;table-layout:fixed;">${rows}</table>`;
  } catch { return ''; }
}

// Hexadec: the day's sixteen tiles, two rows of eight, exactly as the player
// first sees them. Safe to show in full — the four words that use all sixteen
// are the thing you have to find, and seeing the letters is the puzzle, not
// the answer.
function peekHexadecTiles(daysJs, sendISO) {
  try {
    const from = daysJs.match(/DAYS_FROM = "(\d{4}-\d{2}-\d{2})"/);
    const tiles = daysJs.match(/TILES = "([a-z ]+)"/);
    if (!from || !tiles) return '';
    const list = tiles[1].split(' ');
    const i = Math.round((Date.parse(sendISO + 'T12:00:00Z') - Date.parse(from[1] + 'T12:00:00Z')) / 86400000);
    if (!(i >= 0 && i < list.length)) return '';
    const day = list[i];
    if (!/^[a-z]{16}$/.test(day)) return '';

    const cell = ch => `<td width="12.5%" align="center" bgcolor="#fbf5e9" style="background-color:#fbf5e9;border-radius:6px;padding:11px 2px;font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:700;color:#2b2118;">${escapeHtml(ch.toUpperCase())}</td>`;
    const rows = [0, 8].map(k => `<tr>${day.slice(k, k + 8).split('').map(cell).join('')}</tr>`).join('');

    return peekLabel("Today's sixteen") +
      `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate;border-spacing:4px;margin:0 0 16px;table-layout:fixed;">${rows}</table>`;
  } catch { return ''; }
}

// What Word: the hardest of the day's three, shown as the real question with
// its four real options and no answer marked.
function peekWhatWord(questions) {
  try {
    ...
    const opts = q.options.map((o, i) => `
      <tr><td style="padding:0 0 5px;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"><tr>
          <td width="22" valign="top" style="font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:700;color:#3d5588;padding-top:1px;">${letters[i]}</td>
          <td style="font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:1.45;color:#4a4a4a;">${escapeHtml(String(o))}</td>
        </tr></table>
      </td></tr>`).join('');

    const ask = q.prompt || `What does "${q.word}" mean?`;

    const label = q.level === "hard" ? "Today's hardest of the three" : "One of today's three";
    return peekLabel(label) + `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#f7f5f1;border-radius:10px;margin:0 0 16px;"><tr><td style="padding:14px 16px;">
        <div style="font-family:Georgia,'Times New Roman',serif;font-size:15px;font-weight:700;color:#1a1a1a;padding-bottom:10px;">${escapeHtml(ask)}</div>
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">${opts}</table>
      </td></tr></table>`;
  } catch { return ''; }
}

// Whenly: the day's actual events with the year left blank, which is exactly
// what the player is looking at.
function peekWhenly(questions) {
  try {
    const picks = questions.slice(0, 3).filter(q => q && q.question);
    if (!picks.length) return '';

    const rows = picks.map(q => `
      <tr>
        <td valign="middle" style="font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:1.45;color:#4a4a4a;padding:0 10px 7px 0;">${escapeHtml(q.question)}</td>
        <td valign="middle" width="58" align="center" bgcolor="#f0e6d9" style="background-color:#f0e6d9;border-radius:6px;font-family:Georgia,'Times New Roman',serif;font-size:14px;font-weight:700;color:#c9772f;letter-spacing:0.12em;padding:5px 0;">????</td>
      </tr>`).join('');

    return peekLabel(picks.length === 1 ? "Today's event" : "Some of today's events") + `
