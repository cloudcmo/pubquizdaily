...
exports.handler = async function() {
  ...
  try {
    ...
    const previewHtml = buildPreviewWrapper(cleanHtml.replace('%%UNSUB%%', '#'), { subject, fridayISO, cancelUrl, aiIntroNote, aiWhenlyNote, aiWhatWordNote });
...
function buildTeaserHtml({ kicker, headline, intro, hero, statText, fridayISO, whenlyPromo, whatwordPromo }) {
...
function buildPreviewWrapper(innerHtml, { subject, fridayISO, cancelUrl, aiIntroNote, aiWhenlyNote, aiWhatWordNote }) {
  const diag = (aiIntroNote || aiWhenlyNote || aiWhatWordNote) ? `
    <div style="font-size:11px;color:#8f8a82;padding-top:14px;line-height:1.7;border-top:1px solid #333;margin-top:14px;">
      ...
      ${escapeHtml(aiIntroNote || '')}<br>${escapeHtml(aiWhenlyNote || '')}<br>${escapeHtml(aiWhatWordNote || '')}
    </div>` : '';
  const banner = `
