...
module.exports = {
  onPostBuild: async ({ constants, utils }) => {
    ...
    // build-archive.js lives at the repo root; Netlify runs the build from
    // there, so resolve against cwd rather than a brittle relative path.
    const scriptPath = path.join(process.cwd(), 'build-archive.js');
